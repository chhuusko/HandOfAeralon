...
{
    private static readonly HashSet<CharacterClass> _meleeClassSet =
        new() { CharacterClass.Barbarian, CharacterClass.Rogue };
    ...
    private LinePattern _linePattern =
        ScriptableObject.Instantiate(Resources.Load<LinePattern>("AI/LinePattern"));
    private FlamePattern _flamePattern =
        ScriptableObject.Instantiate(Resources.Load<FlamePattern>("AI/FlamePattern"));
    private HousePattern _housePattern =
        ScriptableObject.Instantiate(Resources.Load<HousePattern>("AI/HousePattern"));
    private TriSquarePattern _trisquarePattern =
        ScriptableObject.Instantiate(Resources.Load<TriSquarePattern>("AI/TriSquarePattern"));
    ...
    public AI_Action Evaluate(AI_Context context, List<AI_Action> actions)
    {
        ...
        {
            Debug.LogError(
                $"AI_Evaluator.cs | NO ACTIONS FOUND for {context.Self.name}! Defaulting to standing still.");

            return new AI_Action
            {
                Movement = context.Self.GetCurrentTileComponent()
            };
        }
    ...
    private float EvaluateMovement(AI_Context context, AI_Action action)
    {
        ...
        if (HAZARD_AVOIDANCE > 0f &&
            context.Self.GetCurrentTileComponent() != action.Movement)
        {
            ...
            {
                if (step.GetTileType() == TileType.Lava ||
                    step.GetTileType() == TileType.Poison)
                {
            ...
            context.Allies.Count > 1)
        {
            ...
            {
                int deltaDistance =
                    GetDeltaDistance(context.Self, closestAlly, action.Movement);
        ...
        if (context.Enemies.Any())
        {
            ...
            {
                int deltaDistance =
                    GetDeltaDistance(context.Self, closestEnemy, action.Movement);
                ...
                {
                    int currentDistance =
                        GridExplorer._instance.ManhattanDistance(
                            context.Self.GetCurrentTileIndex(),
                            closestEnemy.GetCurrentTileIndex());

                    if (currentDistance >
                        context.Self.GetMovementPoints() + RANGED_CHARACTER_PREFERRED_RANGE)
                    {
                        modifier = -5f;
                    }
                }
    ...
    private Character FindClosestCharacter(AI_Context context, List<Character> characters)
    {
        ...
        {
            Debug.LogError(
                $"AI_Evaluator.cs | CAN'T CALCULATE closest character for {context.Self.name}!");
            return null;
        }
    ...
    private int GetDeltaDistance(Character a, Character b, CombatGridTile movement)
    {
        int currentDistance =
            GridExplorer._instance.ManhattanDistance(
                a.GetCurrentTileIndex(),
                b.GetCurrentTileIndex());

        int postMoveDistance =
            GridExplorer._instance.ManhattanDistance(
                movement.GetTileIndex(),
                b.GetCurrentTileIndex());
    ...
    private float EvaluateAbilityUsage(AI_Context context, AI_Action action)
    {
        ...
        switch (action.Ability.name)
        {
            ...
            case "ResonantBlast_Ability":       result += ResonantBlast_Ability(context, action);       break;
    ...
    private float Earthquake_Ability(AI_Context context, AI_Action action)
    {
        ...
        var direction = GetDirection(action.Movement, action.Target);
        _housePattern.SetDirection(direction);

        var areaOfEffect =
            _housePattern.CalculateTilesToEffect(action.Target);
    ...
    private float RuptureOfTheWilds_Ability(AI_Context context, AI_Action action)
    {
        ...
        var areaOfEffect =
            _trisquarePattern.CalculateTilesToEffect(action.Target);
    ...
    private float RoarOfTheAncients_Ability(AI_Context context, AI_Action action)
    {
        ...
        var areaOfEffect = DiamondPattern(action.Target, 3);
        ...
        foreach (var tile in areaOfEffect)
        {
            ...
            if (occupant.Exists && occupant.IsEnemy &&
                !occupant.HasStatusEffect<Slowed>())
            {
                hitCount++;
                result += 50f;
            }
    ...
    private float InspiringAnthem_Ability(AI_Context context, AI_Action action)
    {
        ...
        var areaOfEffect = DiamondPattern(action.Target, 2);
        ...
        foreach (var tile in areaOfEffect)
        {
            ...
            if (occupant.Exists && !occupant.IsEnemy &&
                !occupant.HasStatusEffect<Haste>())
            {
                hitCount++;
                result += 50f;
            }
    ...
    private float SongOfRenewal_Ability(AI_Context context, AI_Action action)
    {
        ...
        result *= context.PercentHP;

        if (hitCount == 0 || context.PercentHP < 0.2f)
            result = float.NegativeInfinity;
        ...
        foreach (var tile in areaOfEffect)
        {
            ...
            if (occupant.Exists && occupant.IsEnemy)
            {
                if (occupant.HasStatusEffect<Haste>() ||
                    occupant.HasStatusEffect<Empowered>() ||
                    occupant.HasStatusEffect<Emberwake>() ||
                    occupant.HasStatusEffect<Enraged>() ||
                    occupant.HasStatusEffect<ConduitOfPower>() ||
                    occupant.HasStatusEffect<Fortified>() ||
                    occupant.HasStatusEffect<Sanctified>() ||
                    occupant.HasStatusEffect<Stealth>())
                {
    ...
    private float ResonantBlast_Ability(AI_Context context, AI_Action action)
    {
        float result = 0f;
        int hitCount = 0;
        ...
        if (hitCount == 0 || context.PercentHP > 0.9f)
            result = float.NegativeInfinity;
    ...
    private float DesertsGrasp_Ability(AI_Context context, AI_Action action)
    {
        ...
        foreach (var tile in areaOfEffect)
        {
            ...
            if (occupant.Exists && occupant.IsEnemy)
            {
                ...
                if (!_meleeClassSet.Contains(
                        occupant.Character.GetCharacterClass()))
                    result += PREFER_UNARMORED_TARGETS;
            }
    ...
    private float ThrowingKnives_Ability(AI_Context context, AI_Action action)
    {
        ...
        var areaOfEffect =
            _linePattern.CalculateTilesToEffect(action.Target);
    ...
    private float FlameSurge_Ability(AI_Context context, AI_Action action)
    {
        ...
        var areaOfEffect =
            _flamePattern.CalculateTilesToEffect(action.Target);
    ...
    private float Emberwake_Ability(AI_Context context, AI_Action action)
    {
        ...
        if (!context.StatusEffectManager.ContainsStatusEffect<Emberwake>())
        {
            ...
            foreach (var ability in context.Self.GetAvailableAbilities())
            {
                if ((ability is LightningStorm_Ability ||
                     ability is FlameSurge_Ability) &&
                    !context.Self.IsAbilityCooldownActive(ability))
                {
                    result += 30f;
                }
            }
    ...
    private DirectedAOEPattern.Direction GetDirection(
        CombatGridTile from,
        CombatGridTile to)
    {
        ...
        if (delta == Vector2Int.down)
            return DirectedAOEPattern.Direction.Down;
        if (delta == Vector2Int.left)
            return DirectedAOEPattern.Direction.Left;
        if (delta == Vector2Int.right)
            return DirectedAOEPattern.Direction.Right;

        return DirectedAOEPattern.Direction.None;
    }

    private List<CombatGridTile> DiamondPattern(
        CombatGridTile targetTile,
        int radius)
    {
        ...
        for (int x = -radius; x <= radius; x++)
        {
            for (int y = -radius; y <= radius; y++)
            {
                ...
                if (checkX < 0 || checkX >= CombatGrid._instance.GetGridWidth())
                    continue;
                if (checkY < 0 || checkY >= CombatGrid._instance.GetGridHeight())
                    continue;

                CombatGridTile tile =
                    CombatManager._instance.GetTileComponent(checkX, checkY);
